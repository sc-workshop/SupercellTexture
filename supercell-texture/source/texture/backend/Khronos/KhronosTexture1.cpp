...
#pragma region Constructors
    ...
    KhronosTexture1::KhronosTexture1(
        glInternalFormat format, uint16_t width, uint16_t height, uint8_t* buffer, size_t buffer_size) :
        ...
        construct_buffer(buffer, buffer_size);
    }

    KhronosTexture1::KhronosTexture1(glInternalFormat internal,
                                     glType type,
                                     glFormat format,
                                     uint16_t width,
                                     uint16_t height,
    ...
    KhronosTexture1::KhronosTexture1(RawImage& image, glInternalFormat format) :
        m_internal_format(format) {
        m_format = KhronosTexture::type(format);
        m_width = image.width();
        ...
        set_level_data(image_data, image.depth(), 0);
    }
...
#pragma region Private Functions
    uint32_t KhronosTexture1::read_header(Stream& buffer) {
        ...
        // Some hardcoded/unsuported values
    ...
    void KhronosTexture1::construct_buffer(uint8_t* buffer, size_t size) {
        auto stream = CreateRef<BufferStream>(size);
        Memory::copy(buffer, stream->data(), size);
